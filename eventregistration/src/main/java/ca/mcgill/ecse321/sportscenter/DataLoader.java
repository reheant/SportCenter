...
public class DataLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        for (String[] p : paypals) {
            paypal.save(new PayPal(p[0], customer.findById(Integer.valueOf(p[1])).orElseThrow(), p[2], p[3]));
        }
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(
                "yyyy-MM-dd HH:mm:ss");
        Location l = location
                .save(new Location("Sports Center", 200, Time.valueOf("09:20:00"), Time.valueOf("21:20:00")));
        ...
        course.save(new Course("Soccer Basics", "Learn the basics of soccer including passing, shooting, and tactics.",
        ...
        course.save(new Course("Introduction to Martial Arts",
                "Explore the techniques of martial arts including kicks, punches, and blocks.", CourseStatus.Refused,
                true, 69, 200));
        ...
        for (String[] s : sessions) {
            so = session.save(new Session(LocalDateTime.parse(s[1], formatter), LocalDateTime.parse(s[3], formatter),
