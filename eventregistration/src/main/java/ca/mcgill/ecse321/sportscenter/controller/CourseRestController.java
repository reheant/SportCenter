...
public class CourseRestController {
    ...
    public CourseDto getCourse(@PathVariable("id") int id) throws Exception {
        return DtoConverter.convertToDto(courseService.getCourseById(id));
    }
    ...
    public List<CourseDto> viewFilteredCourses(
        ...
        if (ids == null && keyword == null && status == null && requiresInstructor == null && defaultDuration == null
                && cost == null) {
            return getAllCourses();
        }
        return courseService.viewFilteredCourses(ids, keyword, status, requiresInstructor, defaultDuration, cost)
                .stream().map(p -> DtoConverter.convertToDto(p)).collect(Collectors.toList());
    }
